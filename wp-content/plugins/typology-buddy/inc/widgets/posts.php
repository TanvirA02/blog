...
class Typology_Posts_Widget extends WP_Widget {
	...
	function __construct() {
		$widget_ops = array( 'classname' => 'typology_posts_widget', 'description' => esc_html__( 'Display your posts with this widget', 'typology-buddy' ) );
		$control_ops = array( 'id_base' => 'typology_posts_widget' );
		parent::__construct( 'typology_posts_widget', esc_html__( 'Typology Posts', 'typology-buddy' ), $widget_ops, $control_ops );

		$this->defaults = array(
			'title' => esc_html__( 'Posts', 'typology-buddy' ),
			'style' => 'list',
			'numposts' => 5,
			'category' => array(),
			'auto_detect' => 0,
	...
	function form( $instance ) {
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id( 'title' )); ?>"><?php esc_html_e( 'Title', 'typology-buddy' ); ?>:</label>
		...
		<p>
	   	 	<label for="<?php echo esc_attr($this->get_field_id( 'numposts' )); ?>"><?php esc_html_e( 'Number of posts to show', 'typology-buddy' ); ?>:</label>
	   ...
	   <p>
	   	 <label for="<?php echo esc_attr($this->get_field_id( 'manual' )); ?>"><?php esc_html_e( 'Or choose manually', 'typology-buddy' ); ?>:</label>
		 ...
		 <small class="howto"><?php esc_html_e( 'Specify post ids separated by comma if you want to select only those posts. i.e. 213,32,12,45 Note: you can also choose pages as well as custom post types', 'typology-buddy' ); ?></small>
	   </p>
	  ...
	  <p>
		...
		<label for="<?php echo esc_attr($this->get_field_id( 'auto_detect' )); ?>"><?php esc_html_e( 'Auto detect category', 'typology-buddy' ); ?></label>
		<small class="howto"><?php esc_html_e( 'If sidebar is used on single post template, display posts from current post category ', 'typology-buddy' ); ?></small>
	  </p>

	   <p>
	   	 <label for="<?php echo esc_attr($this->get_field_id( 'tag' )); ?>"><?php esc_html_e( 'Tagged with', 'typology-buddy' ); ?>:</label>
		 ...
		 <small class="howto"><?php esc_html_e( 'Specify one or more tags separated by comma. i.e. life, cooking, funny moments', 'typology-buddy' ); ?></small>
	   </p>
	...
	function widget_orderby( $widget_instance = false, $orderby = false ) {
		...
		$orders = function_exists( 'typology_get_post_order_opts' ) ? typology_get_post_order_opts() : typology_get_post_related_order_opts() ;

		if ( !empty( $widget_instance ) ) { ?>
				<label for="<?php echo esc_attr($widget_instance->get_field_id( 'orderby' )); ?>"><?php esc_html_e( 'Order by:', 'typology-buddy' ); ?></label>
...
?>
				<label for="<?php echo esc_attr($widget_instance->get_field_id( 'category' )); ?>"><?php esc_html_e( 'Choose from:', 'typology-buddy' ); ?></label><br/>
	...
	function widget_style( $widget_instance = false, $current = false ) {
		...
		$styles = array(
			'list' => esc_html__( 'Simple list', 'typology-buddy' ),
			'timeline' => esc_html__( 'Timeline', 'typology-buddy' ),
			'thumbnail' => esc_html__( 'List with thumbnails', 'typology-buddy' )
		);

		if ( !empty( $widget_instance ) ) { ?>
				<label for="<?php echo esc_attr($widget_instance->get_field_id( 'style' )); ?>"><?php esc_html_e( 'Posts style:', 'typology-buddy' ); ?></label>
	...
	function widget_meta( $widget_instance = false, $current = false ) {
		...
		if ( !empty( $widget_instance ) ) : ?>
				<label for="<?php echo esc_attr($widget_instance->get_field_id( 'meta' )); ?>"><?php esc_html_e( 'Display meta data:', 'typology-buddy' ); ?></label><br/>
