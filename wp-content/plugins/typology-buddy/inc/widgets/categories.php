...
class Typology_Category_Widget extends WP_Widget {
	...
	function __construct() {
		$widget_ops = array( 'classname' => 'typology_category_widget', 'description' => esc_html__( 'Display your category list with this widget', 'typology-buddy' ) );
		$control_ops = array( 'id_base' => 'typology_category_widget' );
		parent::__construct( 'typology_category_widget', esc_html__( 'Typology Categories', 'typology-buddy' ), $widget_ops, $control_ops );

		$this->defaults = array(
			'title' => esc_html__( 'Categories', 'typology-buddy' ),
			'categories' => array(),
			'count' => 1
		);
	}
	...
	function widget( $args, $instance ) {
		...
		<ul>
		    ...
		    <?php $cats = buddy_sort_option_items( $cats,  $instance['categories']); ?>
	...
	function form( $instance ) {
		...
		<p>
		<label><?php esc_html_e( 'Choose (re-order) categories:', 'typology-buddy' ); ?></label><br/>
		...
		<small class="howto"><?php esc_html_e( 'Note: Leave empty to display all categories', 'typology-buddy' ); ?></small>
		</p>

		<p>
			<label><input type="checkbox" name="<?php echo esc_attr($this->get_field_name( 'count' )); ?>" value="1" <?php echo checked($instance['count'], 1, true); ?> /><?php esc_html_e( 'Show post count?', 'typology-buddy' ); ?></label>
		</p>
