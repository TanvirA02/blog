	...
	function widget( $args, $instance ) {
		...
		<?php if(!empty($instance['ads'])) : ?>
			...
			<?php
				...
				$target = $instance['no_target_blank'] ? '' : 'target="_blank" rel="noopener"';
		...
		if ( ! in_array( $active_theme, $meks_themes ) ) {
			...
			if (  !get_option('meks_admin_notice_info') || ( get_option('track_transient') && !get_transient( 'meks_admin_notice_time_'. get_current_user_id() ) ) ) {
				...
				<div class="meks-notice notice notice-info is-dismissible">
					<p>
						<?php
							echo sprintf( __( 'You are currently using %1$s theme. Did you know that Meks plugins give you more features and flexibility with one of our <a href="%2$s">Meks themes</a>?', 'meks-easy-ads-widget' ), $all_themes[ $active_theme ], 'https://1.envato.market/4DE2o' );
						?>
					</p>
				</div>
				<?php
